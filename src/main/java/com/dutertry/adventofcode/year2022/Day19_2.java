    ...
    public static void main(String[] args) {
        try(BufferedReader br = AdventUtils.getBufferedReader(19)) {
            ...
            for(int index = 0; index < 3; index++) {
                ...
                int MAX = 32;
                for(int i = 0; i < MAX; i++) {
                    int remainingIteration = MAX - i - 1;
                    ...
                    for(State state : states) {
                        ...
                        for(State nextRobotState : nextRobotStates) {
                            ...
                            int maxExpectedGeodes = nextState.geodeCount + nextState.geodeRobotCount * remainingIteration;
                            if(remainingIteration > 1) {
                                maxExpectedGeodes += (remainingIteration - 1) * remainingIteration / 2;
                            }

                            if(maxExpectedGeodes > maxGeodes) {
