...
public class Day22_2 {
    ...
    public static void main(String[] args) {
        try {
            ...
            for (Brick brick : bricks) {
                ...
                fallingBricks.addAll(getFallingBricks(Collections.singleton(brick), supportedBricks, underBricks));
                for(int level = brick.maxz+1; level <= topz; level++) {
                    final int l = level;
                    Set<Brick> levelBricks = fallingBricks.stream().filter(b -> b.maxz == l).collect(Collectors.toSet());
                    fallingBricks.addAll(getFallingBricks(levelBricks, supportedBricks, underBricks));
                }
                count += fallingBricks.size() - 1;
            }
    ...
    private static Set<Brick> getFallingBricks(Set<Brick> bricks, Map<Brick, Set<Brick>> supportedBricks, Map<Brick, Set<Brick>> underBricks) {
        Set<Brick> fallingBricks = new HashSet<>();
        Set<Brick> supporteds = new HashSet<>();
        for (Brick brick : bricks) {
            supporteds.addAll(supportedBricks.get(brick));
        }
        
        for (Brick supported : supporteds) {
            Set<Brick> unders = new HashSet<>(underBricks.get(supported));
            unders.removeAll(bricks);
            if(unders.isEmpty()) {
